...
manual_expr = None
manual_expr = '(T∨(¬(0)∨(1)))∨(¬((432)∧¬(012))∧(¬(¬2∧3T)∧¬(¬T∧∧∧)))→ '
...
if manual_expr is None:
    dataset_val = 'bool_logic_dataset_val_d5_v1.pkl'
    data_val = Dataset.load_dataset(filename=f'datasets/{dataset_val}')
else:
    dataset_val = 'manual_expr'
    data_val = {'expressions': [manual_expr], 'offset': 0}

outfile = open(f'analysis/analysis_c_{model_name}_on_{dataset_val}_samples_{num_samples}.txt', 'w')
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            test_expression = manual_expr if manual_expr is not None else data_val['expressions'][k]
...
def plot_one_hot_saliency(grads, out_png):
    # one_hot, grads: (1, T, V)
    ...
    truncated = False
    if V > max_cols:
        saliency_plot = saliency[:, :max_cols]  # keep first max_cols vocab cols
        truncated = True
        V_display = max_cols
    else:
        saliency_plot = saliency
        V_display = V

    # we want x-axis = time step (T), y-axis = vocab index (V)
    # imshow expects array with shape (rows, cols) -> (y, x) => transpose saliency_plot
    img = saliency_plot.T  # now shape is (V_display, T) => rows=vocab, cols=time
    ...
    title = f"One-hot saliency heatmap (vocab x time = {V_display}x{T})"
    if truncated:
...
def plot_softmax_heatmap(softmax_logits, out_png):
    ...
    if isinstance(softmax_logits, np.ndarray):
        ...
        try:
            soft = softmax_logits.float().cpu().numpy()
        except Exception:
            soft = softmax_logits.detach().float().cpu().numpy()
...
X = tokenizer.tokenize(output)[:-1]
Y = tokenizer.tokenize(output)[1:]
X = torch.tensor(X, dtype=torch.long, device=device)[None, ...]
Y = torch.tensor(Y, dtype=torch.long, device=device)[None, ...]
loss_mask = torch.zeros_like(Y, dtype=torch.float32)
loss_mask[-1] = 1.0  # only care about the answer token

with ctx:
    logits, loss, grads = model(X, Y, loss_mask=loss_mask, bias=None, mode='pretrain')
...
for layer_idx, recs in attn_records.items():
    if len(recs) == 0:
        ...
        attn_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_layer_{layer_idx}_heads.png"
        plot_attention_heatmap(last_att, attn_png)

softmax_logits = logits.exp()
softmax_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_softmax_heatmap.png"
plot_softmax_heatmap(softmax_logits[0].detach().cpu(), softmax_png)
saliency_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_one_hot_saliency.png"
plot_one_hot_saliency(grads, saliency_png)
...
eos_pos = len(X[0]) - 1
bias[eos_pos, :len_prompt] = 0  # remove prompt attention from the answer token
bias = bias.view(1, 1, block_size, block_size).to(device=device)

with ctx:
    logits, loss, grads = model(X, Y, loss_mask=loss_mask, bias=bias, mode='pretrain')
...
for layer_idx, recs in attn_records.items():
    if len(recs) == 0:
        ...
        attn_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_layer_{layer_idx}_heads_bias.png"
        plot_attention_heatmap(last_att, attn_png)

softmax_logits = logits.exp()
softmax_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_softmax_heatmap_bias.png"
plot_softmax_heatmap(softmax_logits[0].detach().cpu(), softmax_png)
saliency_png = f"analysis/analysis_c_{model_name}_on_{dataset_val}_one_hot_saliency_bias.png"
plot_one_hot_saliency(grads, saliency_png)
outfile.write(f"-2 token softmax bias {softmax_logits[0, -5]}\n")
outfile.write(f"-1 token softmax bias {softmax_logits[0, -4]}\n")
