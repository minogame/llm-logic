...
from model_manba import MambaLM, ManbaConfig
...
exp_name = 'train_manba_000'
train_log = f'log/{exp_name}.log'
...
log_file.write(f"tokens per iteration will be: {tokens_per_iter:,}\n")
log_file.flush()
...
def get_batch(split):
    ...
    if data['offset'] + batch_size > len(data['tokenized_expressions']):
        ...
        log_file.write(f"Resetting {split} dataset offset to 0\n")
        log_file.flush()
...
# model init
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size, d_state=d_state, d_conv=d_conv, expand=expand,
                  bias=bias, vocab_size=None, dropout=dropout) # start with model_args from command line
if init_from == 'scratch':
    # init a new model from scratch
    print("Initializing a new model from scratch")
    log_file.write("Initializing a new model from scratch\n")
    log_file.flush()
    # determine the vocab size we'll use for from-scratch training
    model_args['vocab_size'] = tokenizer.vocab_size
    gptconf = ManbaConfig(**model_args)
    model = MambaLM(gptconf)
elif init_from == 'resume':
    print(f"Resuming training from {out_dir}")
    log_file.write(f"Resuming training from {out_dir}\n")
    log_file.flush()
...
print(f"\nTotal model #parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} M")
log_file.write(f"\nTotal model #parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} M\n")
log_file.flush()
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                if (iter_num) % 2000 == 0:
                    ...
                    log_file.flush()
                    torch.save(checkpoint, os.path.join(out_dir, f'{exp_name}_{iter_num}.pt'))
